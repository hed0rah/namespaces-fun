...
echo ""
echo -e "${CYAN}[*] Holding the namespace open for 60s so you can poke around.${RESET}"
echo -e "    From another terminal, try any of:"
echo -e "      ${GREEN}sudo ip netns exec ${NS_NAME} bash${RESET}        # drop into a shell inside"
echo -e "      ${GREEN}sudo ip netns exec ${NS_NAME} ip addr${RESET}     # see its interfaces"
echo -e "      ${GREEN}sudo ip netns exec ${NS_NAME} ss -tlnp${RESET}    # listening sockets (none yet)"
echo -e "      ${GREEN}sudo lsns -t net${RESET}                           # see it in the namespace list"
